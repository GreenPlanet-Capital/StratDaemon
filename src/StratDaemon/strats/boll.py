...
from StratDaemon.utils.indicators import add_boll_diff
...
class BollStrategy(BaseStrategy):
    def __init__(
        ...
        super().__init__(
            "bollinger", broker, notif, conf, currency_codes, auto_generate_orders, max_amount_per_order, paper_trade, confirm_before_trade
        )

    def execute_buy_condition(
        self, df: DataFrame[CryptoHistorical], order: CryptoLimitOrder
    ) -> bool:
        return df.iloc[-1].close < order.limit_price and df.iloc[-1].boll_diff <= 0.5
    ...
    def transform_df(
        ...
        return add_boll_diff(df, DEFAULT_INDICATOR_LENGTH)
